...
export default function UnhandledAlerts() {
  ...
  .then(response => {
    ...
    function addAlertConfirmation(recEmail, date1, item_list){
      fetch("https://recycleitbackend.herokuapp.com/app/add_alert_conf/?rec_email="+recEmail+"&org_email="+org.email +"&date="+date1+"&item_list="+item_list+"&confirmation=U&response=IN", {
        headers : {
            'Content-Type' : 'application/json'
        },
        method : "GET",
    ...
    .then(response => {
        ...
        console.log(response)
          ...
          alert("alert confirmation sent");
   
    });
    }
    const addToHandledList = (event, id, recEmail, date, item_list) => {
  ...
  .then(response => {
    ...
    if(response.result === 'ok'){
      ...
      addAlertConfirmation(recEmail, date, item_list);
      setTrashAlerts(tempList1);
      addPerformence();
    setShowResult(true);
     ...
     <Grid item xs={12} sm={3}>
          ...
          <Grid xs={12}>
            ...
            <Grid xs={4} sm={2}><CalendarTodayIcon color="secondary" style={{display:"inline"}}/><p style={{display:"inline", marginLeft:"5px"}}>{trash.alert.date}</p></Grid>
                ...
                value={trash.id}
                onClick={(e) => addToHandledList(e,trash.id, trash.alert.recycler.email, trash.alert.date, trash.alert.item_list)}
              >
